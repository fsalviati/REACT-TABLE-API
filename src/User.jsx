...
class TableUser extends Component {
    ...
    render() {
        ...
        const columns = [
            ...
            { field: 'clientContract', headerName: 'Point of Contact', flex: 0.7 },
            { field: 'clientWebsite', headerName: 'Website', flex: 0.7 },
            {
                ...
                renderCell: () => {
                    const handleEditClient = (clientId) => {
                        console.log(rows);
                        console.log(rows.clientId);
                        this.props.history.push({ pathname: `/newclient/${clientId}`, state: clientId });
                    };

                    return <Button onClick={() => {
                        // MATHEWS
                        //How can I pass the row index to get the CLIENTID about the respective line to be sent to handleEditClient()
                        // Below is hardcoded 0, so every time gets the data from the same user. I was using this way as a test and getting console.log
                        handleEditClient(rows[0].clientId)
                    }
                    } style={{
                        color: "rgb(81, 49, 185)",
        ...
        rows = usersArr.map(function (obj, index) {
            return {
                ...
                clientContract: obj.clientContract,
